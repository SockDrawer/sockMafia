...
describe('nouveau dao/MafiaGame', () => {
    ...
    describe('ctor()', () => {
        ...
        it('should store players as provided', () => {
            const obj = {};
            const activeGame = new MafiaGame({
                players: obj
            });
            activeGame._data.players.should.equal(obj);
        });
        ...
        it('should generate players when not provided', () => {
            const activeGame = new MafiaGame({});
            activeGame._data.players.should.eql({});
        });
    ...
    describe('getter livePlayers', () => {
        ...
        it('should be an array of `MafiaUser`s', () => {
            game._data.players = Array.apply(null, Array(20)).map((_, i) => {
                return {
                    username: `user${i}`
                };
            });
        ...
        it('should shuffle the array of `MafiaUser`s', () => {
            ...
            game._data.players = numbers.map((n) => {
                return {
                    username: `user${n}`
                };
            });
    ...
    describe('getter deadPlayers', () => {
        ...
        it('should be an array of `MafiaUser`s', () => {
            Array.apply(null, Array(20)).forEach((_, i) => {
                game._data.players[`user${i}`] = {
                    username: `user${i}`,
                    isAlive: false
                };
            });
        ...
        it('should not include living players', () => {
            const numbers = Array.apply(null, Array(20)).map((_, i) => i);
            const expected = numbers.map((n) => `user${n}`);
            expected.sort();
            numbers.forEach((n) => {
                const name = `user${n}`;
                game._data.players[name] = {
                    username: name,
                    isAlive: false
                };
                game._data.players['1' + name] = {
                    username: '1' + name,
                    isAlive: true
                };
            });
            const result = game.deadPlayers.map(player => player.username);
            result.sort();
    ...
    describe('getter allPlayers', () => {
        ...
        it('should be an array of `MafiaUser`s', () => {
            game._data.players = Array.apply(null, Array(10)).map((_, i) => {
                return {
                    username: `user${i}`
                };
            });
    ...
    describe('getter phases', () => {
        let game = null;
        beforeEach(() => game = new MafiaGame({}));
        it('should return an Array', () => {
            game.phases.should.be.an('Array');
        });
        it('should return phases as set', () => {
            const expected = [];
            for (let i = 0; i < 50; i += 1) {
                expected.push(`key ${Math.random()}`);
            }
            game._data.phases = expected;
            game.phases.should.deep.equal(expected);
        });
        it('should return copy of phases', () => {
            const expected = [];
            for (let i = 0; i < 50; i += 1) {
                expected.push(`key ${Math.random()}`);
            }
            game._data.phases = expected;
            game.phases.should.not.equal(expected);
        });
    });
    ...
    describe('addPlayer()', () => {
        ...
        it('should store user data in live players', () => {
            return game.addPlayer('foobar').then((user) => {
                game._data.players.foobar.should.be.ok;
                game._data.players.foobar.should.be.an('object');
                game._data.players.foobar.should.not.equal(user);
            });
        });
        ...
        it('should reject when player already exists', () => {
            game._data.players.foobar = {};
            return game.addPlayer('foobar').should.be.rejectedWith('E_USER_EXIST');
        });
        it('should reject when player already exists via userslug', () => {
            game._data.players.foobar = {};
            return game.addPlayer('fooBAR').should.be.rejectedWith('E_USER_EXIST');
        });
    ...
    describe('getActions()', () => {
        ...
        it('should return MafiaActions', () => {
            game._data.players.accalia = {
                isAlive: true
            };
            game._data.players.sockbot = {
                isAlive: true
            };
    ...
    describe('revokeAction()', () => {
        ...
        it('should not allow revoking the actions of the dead', () => {
            game._data.players.accalia.isAlive = false;
